...
<head>
	...
	$(function(){
		...
		$(".cart-select").click(function(){
			var chknum = $(".cart-list :checkbox").size();//选项总个数
			var len = $(".cart-select:checked").length; 
			if(!this.checked){
				$(".cart-select-all").prop("checked",false);
			}
			if(chknum==len){
		...
		$(".submit-btn").click(function(){
			window.location.href="getUserAddress";
		});
...
<body>
	...
	<div class="crumb">
		<div class="w">
			<div class="crumb-list">
				<a href="../mainpage/homepage.jsp" class="crumb-item">ecshop</a> <span>></span>
	...
	<div class="cart-wrap w">
		<c:if test="${empty sessionScope.cartVOList}">
			<p class="err-tip">
				<span>您的购物车空空如也，</span> <a href="mainpage/homepage.jsp">立即去购物</a>
			</p>
		</c:if>
		<c:if test="${not empty sessionScope.cartVOList}">
			<div class="cart-header">
				<table class="cart-table">
					<tbody>
						<tr>
							<th class="cart-cell cell-check"><label> <input
									type="checkbox" class="cart-select-all" checked="checked">
				...
				<table class="cart-table" data-product-id="27" data-checked="1">
					<tbody>
						<c:forEach items="${sessionScope.cartVOList}" var="cartVOList">
							<tr>
								<td class="cart-cell cell-check"><input type="checkbox" 
								
								class="cart-select" cartID="${cartVOList.id}" checked="checked">
								</td>
								<td class="cart-cell cell-img"><a
									href="./detail.html?productId=27" target="_blank"> <img
										class="p-img" src="${cartVOList.imgpath}"
										alt="${cartVOList.info}">
								</a></td>
								<td class="cart-cell cell-info"><a class="link p-name"
									href="./detail.html?productId=27" target="_blank"
									style="font-size: 10px;">${cartVOList.info}</a></td>
								<td class="cart-cell cell-price" style="font-size: 10px;">￥${cartVOList.price}</td>
								<td class="cart-cell cell-count"><span class="count-btn"
									data-opera-type="minus">-</span> <input class="count-input"
									data-max="9952" value="${cartVOList.count}"> <span
									class="count-btn" data-opera-type="plus">+</span></td>
								<td class="cart-cell cell-total" style="font-size: 10px;">￥${cartVOList.total}</td>
								<td class="cart-cell cell-opera"><a
									class="link cart-delete" style="font-size: 10px;">删除</a></td>
							</tr>
						</c:forEach>
					</tbody>
				</table>
			...
			<div class="cart-footer clear">
				...
				<div class="submit-con">
					<span>总价：</span> 
					<span class="submit-total">￥${sessionScope.allTotal}</span>  
	...
	<div class=footer>
		<div class=w>
			<div class=links>
				<a class=link href=//www.vip.com />唯品会</a>| <a class=link
					href=//www.taobao.com />淘宝</a>| <a class=link href=//www.jd.com />京东</a>| <a
					class=link href=//www.yhd.com />一号店</a>
			</div>
			<p class=copyright>Copyright © 2017 Designed By lbcto.com All
				Rights Reserved</p>
		</div>
	</div>
</body>
